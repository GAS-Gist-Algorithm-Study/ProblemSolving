...
int CCW(Point ab, Point bc){
    if((ab.x * bc.y - ab.y * bc.x) > 0ll) return 1;
    else if((ab.x * bc.y - ab.y * bc.x) < 0ll) return -1;
    else return 0;
}
...
int main(){
    ...
    while(target <= num){
        if(CCW(convexHull[convexHull.size() - 2], convexHull[convexHull.size() - 1], points[target]) > 0){
            ...
            else convexHull.emplace_back(points[target++]);
        }
    }
